...
using JuLIP, JuLIP.Potentials, QuadGK, ForwardDiff, LinearAlgebra
...
function si110_cluster(sym, L; kwargs...)
    # @assert isodd(L)   # TODO: why?
    atu, b, _, a = si110_plane(sym; kwargs...)
    at = atu * (L, L, 1)
...
"""
function edge110(sym::Symbol, R::Real;
                  truncate=true, cle=:anisotropic, ν=0.25,
                  calc=sw_eq(), symmetrize = nothing,  # switch back to sym = true
                  TOL=1e-4, zDir=1,
                  eos_correction = true,
                  a = cell(bulk(sym, cubic=true))[1,1])

   @assert sym == :Si

   # compute the lattice parameter
   if a == :equilibrate
       atu = bulk(sym, cubic=true, pbc=true)
   ...
   at, b, x0 = si110_cluster(sym, ceil(Int, R/rnn(:Si)+3); a = a)
  ...
  U = xx -> ([CLE.u_edge_isotropic(xx[1]-x0[1], xx[2]-x0[2], b, ν)...; 0.0], nothing)
   ...
   elseif symmetrize == false
      @assert cle == :anisotropic
      ml_displacement!(at, U)
   elseif symmetrize == nothing
      X = positions(at)
      ...
      error("edge110: unknown parameters symmetrize = $(symmetrize)")
   end

   if truncate
      r = [ norm(x[1:2] - x0[1:2]) for x in positions(at) ]
      Ikeep = findall( r .<= R )
      X = positions(at)[Ikeep]
      C = cell(at)
      at = Atoms(:Si, X)
      set_cell!(at, C)
   end
