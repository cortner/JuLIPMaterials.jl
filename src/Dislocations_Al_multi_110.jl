...
module Dislocations_Al_multi_110

using JuLIP
using JuLIP.ASE
using JuLIP.Potentials
using JuLIP.Constraints
using MaterialsScienceTools.Elasticity_110_multi: elastic_moduli, voigt_moduli, fourth_order_basis, sextic_roots, A_coefficients, D_coefficients, little_a, WcbQuad
...
function fcc_edge_geom(s::AbstractString, R;
   ...
   xOrig, yOrig = mat(X12)[1,:], mat(X12)[2,:]
   xOrig, yOrig = x, y
   if eos_correction
      Xmod = eoscorr([x'; y'], -b)
   ...
   if cle == :isotropic
      #uF contains the gradient
      ux, uy, uF = u_edge_isotropic(xmod, ymod, b, ν)
   elseif cle == :anisotropic
      # #uF contains the gradient
      set_pbc!(atu, true)
      atv = bulk("Al", cubic=true) * 4
      Cv = voigt_moduli(calc, atv)
      ux, uy, uF = u_edge(xmod, ymod, b, Cv, a, TOL=TOL)
   else
      error("unknown `cle`")
   end
   ...
   nu = length(x)
   p  = zeros(3,length(x))
   # Compute the shifts at each lattice site
   xP = zeros(nu)
   yP = zeros(nu)
   zP = zeros(nu)
   #Set reference deformation in correct coordinate system
   F2 = 4.05*JMat( [ sqrt(2)/2 0    0;
                 0   1     0;
                 0   0    sqrt(2)/2 ] )
   X2 = 4.05*[ JVec([0.0, 0.0, 0.0]),
         JVec([(1/2)*1/sqrt(2),1/2, 1/(2*sqrt(2))]) ]
   # construct ASEAtoms
   at2 = ASEAtoms(string("Al","2"))
   set_defm!(at2, F2)
   set_positions!(at2, X2)   
   F0 = defm(at2)
   @show F0
   IF0 = inv(F0)
   #for each site, compute the shift
   for i=1:nu
    @show i
    W = WcbQuad()
    tempF22 = [uF[i,1] uF[i,2]; uF[i,3] uF[i,4]]
    if eos_correction
    	dxi = [1 + b * yOrig[i] / (xOrig[i]^2 + yOrig[i]^2) / (2*π)   b * xOrig[i] / (xOrig[i]^2 + yOrig[i]^2) / (2*π)  ;0 1]
    else
	dxi = [1 0; 0 1]
    end
    #Compute the gradient at each site
    F22 = tempF22*inv(dxi)
    F = [F22[1,1] F22[1,2] 0; F22[2,1] F22[2,2] 0; 0 0 0]
    @show F
    @show [uF[i,1] uF[i,2] 0; uF[i,3] uF[i,4] 0; 0 0 0]
    #F = [uF[i,1] uF[i,2] 0; uF[i,3] uF[i,4] 0; 0 0 0]
    F = F + F0
    #@show F
    p[:,i] = W(F)
    @show p[:,i]
  ...
  #Create new atom list with shifts added in
  Y = zeros(3,nu)
  #Can add shift directly to site or add/subtract half to midpoint as in Brian's CB rule
  #This doesn't use Brian's CB rule for test purposes
  Y[1,:], Y[2,:], Y[3,:] = x + xP, y + yP, zP 
  New = [X Y]
  atNew = ASEAtoms(string("Al",2*nu))
  set_positions!(atNew, New)
  set_pbc!(atNew, (false, false, true))
  return atNew, xcore
end
...
function u_edge_isotropic(x, y, b, ν)
    ...
    uxx = b/(2*π) *(-y./(r²)  + 1/(2*(1-ν))*(y./(r²)-2*x.^2 .* y./((r²).^2) ))
    uxy = b/(2*π) *(x./(r²)  + 1/(2*(1-ν))*(x./(r²)-2*y.^2.*x./((r²).^2)  ))
    uyx = b/(2*π) *(-(1-2*ν)/(4-4*ν)*2*x./(r²) - 1/(4-4*ν)*(2*x./(r²) - (x.^2 - y.^2).*2x./((r²).^2)) )
    uyy = 0*b/(2*π) *(-(1-2*ν)/(4-4*ν)*2*y./(r²) - 1/(4-4*ν)*(-2*y./(r²) - (x.^2 - y.^2).*2y./((r²).^2)) )	
    uF = [uxx uxy uyx uyy]
    #uF is a N by 4 matrix with each component being in a column
    return ux, uy, uF
end
...
function u_edge{T}(x, y, b, Cv::Array{T,2}, a; TOL = 1e-4)
   ...
   uxx = real( im/(2*π)*(A[1,2]*(D[3] + D[4]*im)*1./(x+p[2]*y) + A[1,3]*(D[5] + D[6]*im)*1./(x+p[3]*y)   ))

   uxy = real( im/(2*π)*(A[1,2]*(D[3] + D[4]*im)*p[2]./(x+p[2]*y) + A[1,3]*(D[5] + D[6]*im)*p[3]./(x+p[3]*y)   ))

   uyx = real( im/(2*π)*(A[2,2]*(D[3] + D[4]*im)*1./(x+p[2]*y) + A[2,3]*(D[5] + D[6]*im)*1./(x+p[3]*y)   ))
