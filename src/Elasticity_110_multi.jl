...
function DpWcb(F, p, at = bulk("Al", pbc=true), sw = lennardjones_eq())
    set_defm!(at, F)
    X = positions(at)
    X[2] = X[1] + p
    set_positions!(at, X)
...
function DpDpWcb()
    F = 4.05*JMat( [ sqrt(2)/2 0    0;
                 0   1     0;
                 0   0    sqrt(2)/2 ] )
    X = 4.05*[ JVec([0.0, 0.0, 0.0]),
         JVec([(1/2)*1/sqrt(2),1/2, 1/(2*sqrt(2))]) ]
    # construct ASEAtoms
    at = ASEAtoms(string("Al","2"))
    set_defm!(at, F)
    set_positions!(at, X)
...
function WcbQuad()
    ...
    set_positions!(at, X)
    set_pbc!(at, (true, true, true))
    return WcbQuad(DpDpW, pinv(DpDpW), at)
end 
