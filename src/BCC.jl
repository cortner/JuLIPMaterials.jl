...
function screw_111(sym::Symbol, R::Float64;
            ...
            bsign = 1, Nquad=30)::AbstractAtoms
   a0, b0, c0 = lattice_constants_111(sym; calc=calc)
   ...
   # create a cluster
   atu = cell_111(sym, calc=calc)
   at = cluster(atu, R, dims=[1,2], x0=x0)
   at = at * (1,1,layers)
   ...
   elseif soln == :anisotropic
      ...
      ℂ = Ten43{Float64}(ℂ[:])
      disl = CLE.Dislocation(b, t, ℂ, Nquad = Nquad, cut = m0, remove_singularity = true)
      X0 = vecs(X0)
      X = vecs(X)
      X .+= disl.(X0)
   else
