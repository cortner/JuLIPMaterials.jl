...
using JuLIP
using JuLIP.ASE
using MaterialsScienceTools.Elasticity_110: elastic_moduli, voigt_moduli, fourth_order_basis, sextic_roots, A_coefficients, D_coefficients, little_a, xi_solver
...
function fcc_edge_plane(s::AbstractString)
   ...
   xcore =  a/√2*JVec([1/4, sqrt(2)/4, 0])#a/√2 * JVec([1/4, -sqrt(2)/4, 0])#a*sqrt(2)/2 * JVec([1/2, -1/3, 0])  # [1/2, 1/3, 0]
...
function fcc_edge_geom(s::AbstractString, R;
                       truncate=true, cle=:isotropic, ν=0.3325, calc=nothing,
                       TOL=1e-4,zDir=1,perfect=false)
   ...
   #print("xcore : ")
   #print(xcore)
   #print(length(x - xcore[1]))
   #print([x - xcore[1], y - xcore[2]])
   #print("            ")
   #x, y = xi_solver([x - xcore[1], y - xcore[2]])
...
function u_edge{T}(x, y, b, Cv::Array{T,2}, a; TOL = 1e-4)
   ...
   #print(real( im/(2*π)*(A[1,2]*(D[3] + D[4]*im)*log(-.000001+p[2]*-.1) + A[1,3]*(D[5] + D[6]*im)*log(-.000001+p[3]*-.1)   )))
   #print(real( im/(2*π)*(A[1,2]*(D[3] + D[4]*im)*log(.000001+p[2]*-.1) + A[1,3]*(D[5] + D[6]*im)*log(.000001+p[3]*-.1)   )))
   @assert isreal(ux)
   @assert isreal(uy)
